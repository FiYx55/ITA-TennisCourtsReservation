...
import { notificationFetch } from '@src/common/breaker';

// GET /notifications/:userId — get user's notifications
async function byUser(req: Request, res: Response) {
  const r = await notificationFetch(`/notifications/${req.params.userId}`);
  res.status(r.status).json(await r.json());
}

// GET /notifications/:userId/unread/count — unread count
async function unreadCount(req: Request, res: Response) {
  const r = await notificationFetch(`/notifications/${req.params.userId}/unread/count`);
  res.status(r.status).json(await r.json());
}

// PATCH /notifications/:id/read — mark as read
async function markRead(req: Request, res: Response) {
  const r = await notificationFetch(`/notifications/${req.params.id}/read`, { method: 'PATCH' });
  res.status(r.status).json(await r.json());
}

// DELETE /notifications/:id — admin: delete notification
async function delete_(req: Request, res: Response) {
  const r = await notificationFetch(`/notifications/${req.params.id}`, { method: 'DELETE' });
  res.status(r.status);
  if (r.status === 204) {
    res.end();
    return;
...
async function create(req: Request, res: Response) {
  const r = await notificationFetch('/notifications', {
    method: 'POST',
